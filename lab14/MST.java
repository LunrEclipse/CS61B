...
public class MST {
    ...
    public static int[][] mst(int V, int[][] E) {
        ...
        UnionFind connections = new UnionFind(V);
        // FIXME: do Kruskal's Algorithm
        Arrays.sort(E, EDGE_WEIGHT_COMPARATOR);
        int index = 0;
        for (int i = 0; i < E.length && index < result.length; i++) {
            int[] curEdge = E[i];
            if (!connections.samePartition(curEdge[0], curEdge[1])) {
                result[index] = E[i];
                connections.union(curEdge[0], curEdge[1]);
                index++;
            }
        }
        return result;
